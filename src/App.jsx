...
function DBMenuEditor({dishes,save,notify}){
  ...
  const IngredientEditor=({items,onUpdate,type})=>{
    const [newName,setNewName]=useState(""); const [newQty,setNewQty]=useState(""); const [newUnit,setNewUnit]=useState("g");
    const add=()=>{ if(!newName.trim())return; onUpdate([...items,{name:newName.trim(),qty:parseFloat(newQty)||0,unit:newUnit}]); setNewName("");setNewQty("");setNewUnit("g"); };
    return(<div>
      {items.map((ing,ii)=>(<div key={ii} style={{display:"flex",gap:4,marginBottom:4,alignItems:"center"}}>
        <input value={ing.name} onChange={e=>onUpdate(items.map((x,xi)=>xi===ii?{...x,name:e.target.value}:x))} style={{flex:2,padding:"6px 8px",border:"1px solid #E0E0E0",borderRadius:6,fontSize:13}}/>
        <input value={ing.qty||""} onChange={e=>onUpdate(items.map((x,xi)=>xi===ii?{...x,qty:parseFloat(e.target.value)||0}:x))} style={{width:50,padding:"6px 4px",border:"1px solid #E0E0E0",borderRadius:6,fontSize:13,textAlign:"center"}} placeholder="量"/>
        <input value={ing.unit} onChange={e=>onUpdate(items.map((x,xi)=>xi===ii?{...x,unit:e.target.value}:x))} style={{width:50,padding:"6px 4px",border:"1px solid #E0E0E0",borderRadius:6,fontSize:13}}/>
        <button onClick={()=>onUpdate(items.filter((_,xi)=>xi!==ii))} style={{padding:"4px 8px",background:"#FFEBEE",color:"#C62828",border:"none",borderRadius:5,fontSize:12}}>×</button>
      </div>))}
      <div style={{display:"flex",gap:4,marginTop:6}}>
        <input value={newName} onChange={e=>setNewName(e.target.value)} placeholder="名前" style={{flex:2,padding:"6px 8px",border:"1px solid #E0E0E0",borderRadius:6,fontSize:13}}/>
        <input value={newQty} onChange={e=>setNewQty(e.target.value)} placeholder="量" style={{width:50,padding:"6px 4px",border:"1px solid #E0E0E0",borderRadius:6,fontSize:13,textAlign:"center"}}/>
        <input value={newUnit} onChange={e=>setNewUnit(e.target.value)} style={{width:50,padding:"6px 4px",border:"1px solid #E0E0E0",borderRadius:6,fontSize:13}}/>
        <button onClick={add} style={{padding:"4px 10px",background:"#1565C0",color:"white",border:"none",borderRadius:5,fontSize:12,fontWeight:700}}>追加</button>
      </div>
    </div>);
  };

  const EntryForm=({data,onChange})=>(
    <div>
      <Lbl>カテゴリ（最大2つ）</Lbl>
      ...
      {data.variants.map((v,vi)=>(
        <div key={v.variantId} style={{background:"#F7F8FA",borderRadius:10,padding:"12px",marginBottom:8}}>
          ...
          <Lbl>食材</Lbl>
          <IngredientEditor items={v.ingredients||[]} onUpdate={ings=>{ const vs=[...data.variants]; vs[vi]={...vs[vi],ingredients:ings}; onChange({...data,variants:vs}); }} type="ingredient"/>
          <Lbl style={{marginTop:8}}>調味料</Lbl>
          <IngredientEditor items={v.seasonings||[]} onUpdate={seas=>{ const vs=[...data.variants]; vs[vi]={...vs[vi],seasonings:seas}; onChange({...data,variants:vs}); }} type="seasoning"/>
        </div>
      ))}
